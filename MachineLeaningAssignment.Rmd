...
We can load both data files and check the data. The last column "classe" contains letters which code the preformance quality. I'll start with loading packages I need.
...
The next step is less obvious: by checking out the "testing" data I've discovered a lot of empty variables which have values in "training" data. Usually they are read as logical with nonexisting values and some as characters.
We can check the amount of numeric variables in the test set and see that the number of meanful predictors is fewer than 40%.
```{r}
