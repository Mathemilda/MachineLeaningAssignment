...
```
To maintain the data consistency I will remove non numeric variables from both sets for my work. The new data sets with numeric predictors will be called "trai" and "test", respectfully.
```{r, cache=TRUE}
...
Let us concentrate on our goal. We need to find ways to distinguish type "A", which is a correct way to perform, from others, which represent different mistakes. I'm going to pick up predictors for which their distributions of  "A" values are notably more compact than at least one the variables and it is placed somehow higher or lower on its corresponding plot. For example, box plots on the picture in the lower right corner fit to this requirement, which corresponds to 12th predictor.
Sometimes we can have problems with outliners, messing up the whole picture.

I will not show other box plots, since I'm not allowed to use more then 5 pictures, and the number of my pictures is questionable already. 
...
"accel_forearm_y" [48].  In addition we need the "classe" variable to be treated as a factor.
...
```
I will use "random forest" algorithm option from "caret" package. Since it uses random numbers, I need to set up my "random number seed".
```{r, cache=TRUE, results = 'hide'}
 seed.set=7
...
```
At last, we compare predictions on the validation set:
```{r}
